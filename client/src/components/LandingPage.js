...
import { Search, ShoppingCart, User, Heart, Star, LayoutDashboard, Settings } from 'lucide-react';
...
const LandingPage = () => {
  ...
  const categories = [
    'Decor', 'Arts', 'Clothing', 'Painting', 'Accessories', 'Jewelry', 'Essentials'
  ];
    ...
    <div className="min-h-screen bg-gray-50 font-inter">
            ...
            <>
              ...
              {user && user.role === 'admin' && (
                <Link to="/admin/panel" className="flex items-center text-gray-700 hover:text-indigo-700 transition-colors duration-200 rounded-md p-2">
                  <Settings size={20} className="mr-1" /> Admin Panel
                </Link>
              )}
      ...
      <footer className="bg-gray-800 text-white py-6 mt-10 rounded-t-lg shadow-inner">
        <div className="container mx-auto text-center">
          ...
          {/* Moved Information section here */}
          <div className="mt-4 pt-4 border-t border-gray-700 flex flex-col sm:flex-row justify-center space-y-2 sm:space-y-0 sm:space-x-4">
            <Link to="/about" className="text-gray-300 hover:text-white transition-colors duration-200">About Us</Link>
            <Link to="/terms" className="text-gray-300 hover:text-white transition-colors duration-200">Terms & Conditions</Link>
            <Link to="/feedback" className="text-gray-300 hover:text-white transition-colors duration-200">Feedback</Link>
          </div>
        </div>
      </footer>
    </div>
